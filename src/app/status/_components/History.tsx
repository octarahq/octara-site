...
export default function History({ status }: { status?: OctaraStatus | null }) {
  ...
  const items = useMemo(() => {
    ...
    > = {
      scheduled: {
        label: t("status.history.maintenance.scheduled"),
        className: "bg-slate-100 text-slate-500",
      },
      "in-progress": {
        label: t("status.history.maintenance.inProgress"),
        className: "bg-indigo-100 text-indigo-700",
      },
      completed: {
        label: t("status.history.maintenance.completed"),
          ...
          items.map((item) => (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${item.statusClass}`}
                >
                  {item.statusLabel}
                </span>
              </div>
