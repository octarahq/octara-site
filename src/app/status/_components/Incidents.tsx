...
export default function Incidents({
  ...
  > = {
    investigating: {
      label: t("status.incidents.label.investigating"),
      className: "bg-amber-100 text-amber-700",
    },
    identified: {
      label: t("status.incidents.label.identified"),
      className: "bg-orange-100 text-orange-700",
    },
    monitoring: {
      label: t("status.incidents.label.monitoring"),
      className: "bg-blue-100 text-blue-700",
    },
    resolved: {
      label: t("status.incidents.label.resolved"),
    ...
    return (
      <section className="mb-12">
        ...
        <div className="rounded-2xl border-2 border-dashed border-slate-200 dark:border-primary/20 bg-slate-50/50 dark:bg-primary/5 p-12 flex flex-col items-center justify-center text-center">
          <span className="material-symbols-outlined text-4xl text-slate-300 dark:text-primary/20 mb-4">
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${statusInfo.className}`}
                >
                  {statusInfo.label}
                </span>
              </div>
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-3">
                {t("status.incidents.lastUpdated", { when: dateLabel })}
              </p>
            </div>
          );
        })}
