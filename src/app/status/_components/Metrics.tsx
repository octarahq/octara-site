...
export default function Metrics({
  ...
  const computeAvailability = () => {
    if (metrics && typeof metrics.availability === "number")
  ...
  const computeAvgResponse = () => {
    if (metrics && typeof metrics.avgResponseMs === "number")
      return metrics.avgResponseMs;
    return null;
  };

  const computeDaysWithout = () => {
    if (metrics && typeof metrics.daysWithoutIncident === "number")
      return metrics.daysWithoutIncident;
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      <div className="bg-slate-50 dark:bg-primary/5 p-5 rounded-xl border border-slate-200 dark:border-primary/10">
        <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">
          {t("status.metrics.availability")}
        </p>
        <p className="text-2xl font-bold">{availability}%</p>
      </div>
      <div className="bg-slate-50 dark:bg-primary/5 p-5 rounded-xl border border-slate-200 dark:border-primary/10">
        <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">
          {t("status.metrics.daysWithout", {
            days: String(daysWithoutIncident ?? 0),
          })}
        </p>
        <p className="text-2xl font-bold">
          {daysWithoutIncident !== null
            ? `${daysWithoutIncident} ${t("status.metrics.daysSuffix")}`
            : t("status.metrics.none")}
        </p>
      </div>
      <div className="bg-slate-50 dark:bg-primary/5 p-5 rounded-xl border border-slate-200 dark:border-primary/10">
        <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">
          {t("status.metrics.responseTime")}
        </p>
        <p className="text-2xl font-bold">
          {avgResponseMs !== null
            ? `${avgResponseMs}ms`
